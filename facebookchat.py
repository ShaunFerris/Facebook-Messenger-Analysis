...
class FacebookChat:
    ...
    def __init__(self, chat_file: TextIO):
        ...
        for msg in self.chat_contents:
            msg["sender_name"] = (
                msg["sender_name"].encode("raw_unicode_escape").decode("utf-8")
            )
            if "content" in msg:
                msg["content"] = (
                    msg["content"].encode("raw_unicode_escape").decode("utf-8")
                )
            ...
            if "call_duration" not in msg and "content" in msg:
                if search_term.lower() in msg["content"].lower():
                    timestamp = datetime.fromtimestamp(
                        msg["timestamp_ms"] / 1000, timezone.utc
                    ).astimezone()
    ...
    def voice_calls_analysis(self) -> tuple[dict[str, int], int]:
        ...
        total_duration = 0
        for msg in self.chat_contents:
            if "call_duration" in msg and msg["content"] != "The video chat ended.":
    ...
    def emoji_by_party(self) -> dict[str, list[tuple[str, int]]]:
        ...
        for party in emojis_by_party:
            ...
            emojis_by_party[party] = {
                emoji.emojize(emoji.demojize(e)): c
                for e, c in emojis_by_party[party].items()
                if c
            }
            emojis_by_party[party] = sorted(
                emojis_by_party[party].items(), key=lambda x: x[1], reverse=True
            )
    ...
    def images_by_sender(self) -> dict[str, list[dict[str, str]]]:
        ...
        for msg in self.chat_contents:
            if "photos" in msg:
                ...
                sorted_images[msg["sender_name"]].append(
                    {sent_time_str: image_filenames}
                )
